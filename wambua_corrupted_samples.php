...
$w_c_e= explode(",", $wambua_corrupted_employees);
$illegal="/[()+=-%<>]/"
 ?>
<table style="border:2px solid black; padding:2px">
  <thead>
    <?php for($i=0;$i<8;$i++):?>
        ...
        echo (preg_replace('/[()0-9>+=\/]+/', '',ucwords(strtolower($w_c_e[$i])))) ; ?></th>
    <?php endfor; ?>
  </thead>
  <tbody>

    <?php for($j=1;$j<(count($w_c_e)-8)/8;$j++):?>

      <tr style="border:1px solid black;">
    <?php for($i=1;$i<9;$i++):?>
    <td style="border:1px solid black;" ><?php
    if ($i==3||$i==2||$i==5) {
      $st=str_replace('oo', 'o',preg_replace("/[^a-zA-Z]/",'',ucwords(strtolower($w_c_e[8*$j+$i]))));
    }else if ($i==7) {

      $st=str_replace('oo', 'o',preg_replace('/[@%()>+=\/\^\-]+/', '',ucwords(strtolower($w_c_e[8*$j+$i]))));
    ...
    $st=(preg_replace('/[@%()0-9>+=\/\^\-]+/', '',ucwords(strtolower($w_c_e[8*$j+$i]))));
  }
    echo $st; ?>
    </td>
    <?php endfor; ?>
